...
extension Defaults {
    static func encoded() -> String? {
        ...
        let config = Config(bundleId: "com.lilypad.Lilypad",
                            version: version,
                            shortcuts: shortcuts,
    ...
    static func loadFromSupportDir() {
        if let lilypadSupportURL = getSupportDir()?
            .appendingPathComponent("Lilypad", isDirectory: true) {

            let configURL = lilypadSupportURL.appendingPathComponent("LilypadConfig.json")
                        
            let exists = try? configURL.checkResourceIsReachable()
            if exists == true {
                load(fileUrl: configURL)
                do {
                    let newFilename = "LilypadConfig\(timestamp()).json"
                    
                    try FileManager.default.moveItem(atPath: configURL.path, toPath: lilypadSupportURL.appendingPathComponent(newFilename).path)
                } catch {
                    do {
                        try FileManager.default.removeItem(at: configURL)
                    } catch {
                        AlertUtil.oneButtonAlert(question: "Error after loading from Support Dir", text: "Unable to rename/remove LilypadConfig.json from \(lilypadSupportURL) after loading.")
                    }
                }
            }
        }
