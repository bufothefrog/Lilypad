...
class GridLayoutManager {
    ...
    static func execute(parameters: ExecutionParameters) -> Bool {
        ...
        // records lastLilypadActions for repeat/last-action detection). applyGridRect
    ...
    /// Overwrite `lastLilypadActions[windowId]` with `action` (a `gridMove<dir>`)
    ...
    private static func recordGridMove(_ action: WindowAction, windowElement: AccessibilityElement, windowId: CGWindowID) {
        ...
        let newCount: Int
        if let last = AppDelegate.windowHistory.lastLilypadActions[windowId], last.action == action {
    ...
    private static func handleWall(direction: GridCalculation.Direction,
        ...
        let lastWasSameMove = AppDelegate.windowHistory.lastLilypadActions[windowId]?.action == action
        ...
        case .none:
            ...
            NSSound.beep()
        case .minimize:
            // AX-minimize the window directly (Lilypad has no minimize action).
    ...
    ///                     Lilypad, so up reuses `previousDisplay` and down reuses
...
/// in Lilypad (`EnhancedUI`, `TodoSidebarSide`, `TodoSidebarWidthUnit`): `0` is
