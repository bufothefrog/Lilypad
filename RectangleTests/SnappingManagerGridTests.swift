...
//  `SnappingManager.gridModeEngaged(modifierFlags:
//  activationModifierRawValue:spanModifierRawValue:)`. Grid mode engages during a
//  drag when the held device-independent modifiers, with the span modifier bits
//  removed, equal the activation modifier — the sole gate that chooses whether a
//  drag snaps to the grid. These tests cover the modifier-held/not matrix,
//  INCLUDING the M6 span-coexistence cases (Shift, Shift+span both engage;
//  Shift+other does not; span alone does not), so a regression that breaks grid
//  engagement, or span coexistence, fails here.
//
...
class SnappingManagerGridTests: XCTestCase {
    ...
    private func engaged(_ flags: NSEvent.ModifierFlags, modifier: Int, span: Int? = nil) -> Bool {
    ...
    // MARK: - Engaged iff the activation modifier is held (span tolerated)

    func testModifierHeld() {
        XCTAssertTrue(engaged(.shift, modifier: shift))
    }

    func testModifierNotHeld() {
        XCTAssertFalse(engaged([], modifier: shift))
    }

    func testWrongModifierHeld() {
        // Command held, but Shift is the activation modifier → not engaged.
        XCTAssertFalse(engaged(.command, modifier: shift))
    }

    func testIgnoresNonDeviceIndependentBits() {
        ...
        XCTAssertTrue(engaged(flags, modifier: shift))
    }
    ...
    func testActivationPlusSpanStillEngages() {
        ...
        XCTAssertTrue(engaged([.shift, .option], modifier: shift))
    }
    ...
    func testSpanAloneDoesNotEngage() {
        ...
        XCTAssertFalse(engaged(.option, modifier: shift))
    }
    ...
    func testActivationPlusSpanPlusOtherDoesNotEngage() {
        ...
        XCTAssertFalse(engaged([.shift, .option, .command], modifier: shift))
    }
    ...
    func testActivationPlusNonSpanExtraDoesNotEngage() {
        ...
        XCTAssertFalse(engaged([.shift, .command], modifier: shift))
    }
    ...
    func testSpanEqualsActivationCollapsesToExactMatch() {
        ...
        XCTAssertTrue(engaged(.shift, modifier: shift, span: shift))
        XCTAssertFalse(engaged([.shift, .option], modifier: shift, span: shift))
    }
    ...
    // MARK: - Zero activation modifier means "every drag engages"

    func testZeroModifierEngagesOnFlagAlone() {
        XCTAssertTrue(engaged([], modifier: 0))
        XCTAssertTrue(engaged(.shift, modifier: 0))
        // Even with the span modifier held, a zero activation engages.
        XCTAssertTrue(engaged(.option, modifier: 0))
    }
