# Lilypad Terminal Commands for Hidden Preferences

The preferences window is purposefully slim, but there's a lot that can be modified via Terminal. After executing a terminal command, restart the app as these values are loaded on application startup. A number of the commands below are for configuring keyboard shortcuts, and you'll need to know the key code and modifier flags integer values. You can use the free [key codes app](https://apps.apple.com/us/app/key-codes/id414568915) to help with this.
...
```bash
defaults write com.lilypad.Lilypad alternateDefaultShortcuts -bool true
```
...
```bash
defaults write com.lilypad.Lilypad subsequentExecutionMode -int 2
```
...
```bash
defaults write com.lilypad.Lilypad centerHalfCycles -int 1
```
...
```bash
defaults write com.lilypad.Lilypad resizeOnDirectionalMove -bool true
```
...
```bash
defaults write com.lilypad.Lilypad stageSize -float <VALUE>
```

To set it to a proportion of your screen's width, set it to a value between 0 and 1.

```bash
defaults write com.lilypad.Lilypad stageSize -float <VALUE_BETWEEN_0_AND_1>
```
...
```bash
defaults write com.lilypad.Lilypad todo -int 1
```
...
```bash
defaults write com.lilypad.Lilypad snapModifiers -int 1048576
```
...
```bash
defaults write com.lilypad.Lilypad almostMaximizeHeight -float <VALUE_BETWEEN_0_&_1>
```

```bash
defaults write com.lilypad.Lilypad almostMaximizeWidth -float <VALUE_BETWEEN_0_&_1>
```
...
```bash
defaults write com.lilypad.Lilypad specified -dict-add keyCode -float 8 modifierFlags -float 1966080
```

```bash
defaults write com.lilypad.Lilypad specifiedHeight -float 1050
defaults write com.lilypad.Lilypad specifiedWidth -float 1680
```
...
```bash
defaults write com.lilypad.Lilypad centerProminently -dict-add keyCode -float 8 modifierFlags -float 1835305
```
...
```bash
defaults write com.lilypad.Lilypad topLeftNinth -dict-add keyCode -float 18 modifierFlags -float 917504
```
...
```bash
defaults write com.lilypad.Lilypad topLeftEighth -dict-add keyCode -float 18 modifierFlags -float 917504
```
...
```bash
defaults write com.lilypad.Lilypad topLeftThird -dict-add keyCode -float 18 modifierFlags -float 917504
```
...
```bash
defaults write com.lilypad.Lilypad doubleWidthRight -dict-add keyCode -float 124 modifierFlags -float 11403555
```
...
```bash
defaults write com.lilypad.Lilypad cascadeActiveApp -dict-add keyCode -float 2 modifierFlags -float 393475
```
...
```bash
defaults write com.lilypad.Lilypad footprintAlpha -float <VALUE_BETWEEN_0_&_1>
```

Change the border width. Default is 2 (used to be 1).

```bash
defaults write com.lilypad.Lilypad footprintBorderWidth -float <NUM_PIXELS>
```

Disable the fade.

```bash
defaults write com.lilypad.Lilypad footprintFade -int 2
```

Change the color.

```bash
defaults write com.lilypad.Lilypad footprintColor -string "{\"red\":0,\"blue\":0.5,\"green\":0.5}"
```

Change the animation duration. The value is a multiplier. Default is 0 (no animation).

```bash
defaults write com.lilypad.Lilypad footprintAnimationDurationMultiplier -float <MULTIPLIER>
```
...
```bash
defaults write com.lilypad.Lilypad centeredDirectionalMove -int 2
```
...
```bash
defaults write com.lilypad.Lilypad minimumWindowWidth -float <VALUE_BETWEEN_0_&_1>
```

```bash
defaults write com.lilypad.Lilypad minimumWindowHeight -float <VALUE_BETWEEN_0_&_1>
```
...
```bash
defaults write com.lilypad.Lilypad sizeOffset -float <NUM_PIXELS>
```
...
```bash
defaults write com.lilypad.Lilypad curtainChangeSize -int 2
```
...
```bash
defaults write com.lilypad.Lilypad largerWidth -dict-add keyCode -float 30 modifierFlags -float 786432
defaults write com.lilypad.Lilypad smallerWidth -dict-add keyCode -float 33 modifierFlags -float 786432
```
...
```bash
defaults write com.lilypad.Lilypad largerHeight -dict-add keyCode -float 30 modifierFlags -float 917504
defaults write com.lilypad.Lilypad smallerHeight -dict-add keyCode -float 33 modifierFlags -float 917504
```

## Disabling window restore when moving windows

```bash
defaults write com.lilypad.Lilypad unsnapRestore -int 2
```
...
```bash
defaults write com.lilypad.Lilypad snapEdgeMarginTop -int 10
defaults write com.lilypad.Lilypad snapEdgeMarginBottom -int 10
defaults write com.lilypad.Lilypad snapEdgeMarginLeft -int 10
defaults write com.lilypad.Lilypad snapEdgeMarginRight -int 10
```
...
```bash
defaults write com.lilypad.Lilypad screenEdgeGapTop -int 10
defaults write com.lilypad.Lilypad screenEdgeGapBottom -int 10
defaults write com.lilypad.Lilypad screenEdgeGapLeft -int 10
defaults write com.lilypad.Lilypad screenEdgeGapRight -int 10
```
...
```bash
defaults write com.lilypad.Lilypad screenEdgeGapTopNotch -int 5
```
...
```bash
defaults write com.lilypad.Lilypad screenEdgeGapsOnMainScreenOnly -bool true
```
...
```bash
defaults write com.lilypad.Lilypad ignoredSnapAreas -int 1
```

To disable the Top Half and Bottom Half snap areas, the bit field would be 1111 0000 0000, or 3840

```bash
defaults write com.lilypad.Lilypad ignoredSnapAreas -int 3840
```
...
```bash
defaults write com.lilypad.Lilypad applyGapsToMaximize -int 2
```

To disable the gaps for "Maximize Height", execute:

```bash
defaults write com.lilypad.Lilypad applyGapsToMaximizeHeight -int 2
```
...
```bash
defaults write com.lilypad.Lilypad sixthsSnapArea -bool true
```
...
```bash
defaults write com.lilypad.Lilypad moveCursor -int 1
```
...
```bash
defaults write com.lilypad.Lilypad missionControlDragging -int 2
```

Change the allowed off-screen distance. The value is in pixels and is tied to the speed. Default is 25.

```bash
defaults write com.lilypad.Lilypad missionControlDraggingAllowedOffscreenDistance -float <DISTANCE>
```

Change the disallowed duration. The value is in milliseconds. Default is 250.

```bash
defaults write com.lilypad.Lilypad missionControlDraggingDisallowedDuration -int <DURATION>
```

## Change the behavior of double-click window title bar

To change the action ([list](https://github.com/bufothefrog/Lilypad/blob/main/Lilypad/WindowAction.swift)):

```bash
defaults write com.lilypad.Lilypad doubleClickTitleBar -int <ACTION_ID + 1>
```

To disable restore when double-clicked again:

```bash
defaults write com.lilypad.Lilypad doubleClickTitleBarRestore -int 2
```

To disable double-click window title bar only for specific bundle ids (in example, Outlook):

```bash
defaults write com.lilypad.Lilypad doubleClickTitleBarIgnoredApps -string "[\"com.microsoft.Outlook\"]"
```
...
```bash
defaults write com.lilypad.Lilypad screensOrderedByX -int 1
```